...
class Clause:
    ...
    __slots__ = ('idx', 'positive', 'negative', 'key_positive', 'satisfy')
...
class ConstrainedHypothesis:
    ...
    def __init__(self,
        ...
        self.verb_idx = len(constraint_list) - 1
    ...
    def is_pos_valid(self) -> bool:
        if len(self.orders) >= 2 and self.verb_idx not in self.orders:
            return False
        return True

    def allowed(self) -> Set[int]:
        tokens = self.positive_state.allowed()
        if len(self.orders) == 1 and self.verb_idx not in self.orders:
            tokens = [x[0] for x in self.clauses[self.verb_idx].positive]
        elif not self.orders and self.in_process and self.verb_idx not in self.in_process:
            tokens = [x[0] for x in self.clauses[self.verb_idx].positive] + [x[-1][0] for x in self.literal_in_process]
        return set(tokens)
    ...
    def phrase_to_look_ahead(self) -> List[Phrase]:
        ...
        """
        if len(self.orders) == 1 and self.verb_idx not in self.orders:
            look_ahead_phrases = self.clauses[self.verb_idx].key_positive
        elif not self.orders and self.in_process and self.verb_idx not in self.in_process:
            look_ahead_phrases = self.clauses[self.verb_idx].key_positive
        else:
            look_ahead_phrases = []
