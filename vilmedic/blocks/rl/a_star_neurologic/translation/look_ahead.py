...
def _generate_greedy(
    ...
    for t in range(look_ahead_step):
        ...
        for j, (phrase, phrase_mask) in enumerate(phrases_idx_mask):
            if torch.sum(phrase_mask[:, t]).item():
                ...
                top_scores, top_tokens = torch.topk(log_prob, THRESHOLD, dim=1, largest=True, sorted=True)
                is_valid = (phrase_score >= torch.min(top_scores, dim=-1)[0]).int()

                if len(phrase) > 1:
                    phrase_score = phrase_score[:, None]
                    phrase_past = past
                    phrase_input_ids = input_ids

                    for m in range(len(phrase[:-1])):
                        phrase_input_ids = torch.cat([phrase_input_ids, input_ids.new([phrase[m]])[None, :].expand(batch_size, -1)], dim=-1)
                        phrase_inputs = self.prepare_inputs_for_generation(
                            phrase_input_ids, past=phrase_past, attention_mask=attention_mask, use_cache=use_cache,
                            **model_specific_kwargs
                        )
                        phrase_outputs = self(**phrase_inputs)
                        follow_logits = phrase_outputs[0][:, -1, :]
                        phrase_past = phrase_outputs[1]

                        follow_log_prob = F.log_softmax(follow_logits, dim=-1)
                        phrase_score = torch.cat([phrase_score, follow_log_prob[:, phrase[m + 1]][:, None]], dim=-1)
    ...
    look_ahead_scores = -torch.log(1 - torch.exp(look_ahead_scores))
        ...
        for j, (phrase, phrase_mask) in enumerate(phrases_idx_mask):
            if torch.sum(phrase_mask[:, t]).item():
                ...
                if len(phrase) > 1:
                    ...
                    for m in range(len(phrase[:-1])):
                        phrase_input_ids = torch.cat([phrase_input_ids, input_ids.new([phrase[m]])[None, :].expand(input_ids.shape[0], -1)], dim=-1)
                        phrase_inputs = self.prepare_inputs_for_generation(
                            phrase_input_ids, past=phrase_past, attention_mask=attention_mask, use_cache=use_cache,
                            **model_specific_kwargs
                        )
    ...
    for t in range(look_ahead_step):
        ...
        for j, (phrase, phrase_mask) in enumerate(phrases_idx_mask):
            if torch.sum(phrase_mask[:, t]).item():
                ...
                top_scores, top_tokens = torch.topk(scores, THRESHOLD, dim=1, largest=True, sorted=True)
                is_valid = (phrase_score >= torch.min(top_scores, dim=-1)[0]).int()
